"""
Mohamed_Samatar_CST2213_BI_Final_Project_Submission.py
=======================================================
...
HOW TO RUN
----------
1. Put this file and 2020_Tabular_Transportation_Collision_Data.csv
...
4. Run:  streamlit run Mohamed_Samatar_CST2213_BI_Final_Project_Submission.py
...
# Columns present in BOTH cleaned_ottawa_collision_data.csv AND
# dashboard_ready_collision_data_no_unknown.csv (the two "already engineered"
# exports this app produces). If a file has all of these, it's already been
# through the cleaning pipeline once and doesn't need to go through it again.
REQUIRED_ENGINEERED_COLUMNS = [
    "Accident_Date", "Hour", "Env", "Light_Cond", "Road", "Traffic",
    "Impact", "Loc_Type", "Severity", "Injury_Event",
    "No__of_Vehicles", "No__of_Bicycles", "No__of_Motorcycles", "No__of_Pedestrians",
    "Latitude", "Longitude",
]
...
def detect_format(df: pd.DataFrame) -> str | None:
    """Return 'raw', 'engineered', or None (unrecognized)."""
    if all(c in df.columns for c in REQUIRED_RAW_COLUMNS):
        return "raw"
    if all(c in df.columns for c in REQUIRED_ENGINEERED_COLUMNS):
        return "engineered"
    return None


def build_schema_error(df: pd.DataFrame) -> str:
    missing_raw = [c for c in REQUIRED_RAW_COLUMNS if c not in df.columns]
    missing_eng = [c for c in REQUIRED_ENGINEERED_COLUMNS if c not in df.columns]
    return (
        "This file doesn't match a format this app recognizes.\n\n"
        "It accepts THREE kinds of files:\n"
        "1. The original raw file (2020_Tabular_Transportation_Collision_Data.csv)\n"
        "   \u2192 missing for this option: " + (", ".join(missing_raw) if missing_raw else "none") + "\n\n"
        "2. The cleaned file (cleaned_ottawa_collision_data.csv)\n"
        "   \u2192 missing for this option: " + (", ".join(missing_eng) if missing_eng else "none") + "\n\n"
        "3. The dashboard-ready file (dashboard_ready_collision_data_no_unknown.csv)\n"
        "   \u2192 missing for this option: " + (", ".join(missing_eng) if missing_eng else "none") + "\n\n"
        "Please upload one of these three files without renaming its columns."
    )


def clean_raw(df: pd.DataFrame) -> pd.DataFrame:
    """Full Phase 1/2 cleaning + feature engineering, for the ORIGINAL raw export."""
    df = df.copy()
...
def adapt_engineered(df: pd.DataFrame) -> pd.DataFrame:
    """Light-touch adapter for files that are ALREADY cleaned/engineered
    (cleaned_ottawa_collision_data.csv or dashboard_ready_collision_data_no_unknown.csv).
    Fills in only what might be missing so every downstream function
    (filters, charts, model training, hotspot clustering) sees the same
    column set regardless of which of the three files was loaded."""
    df = df.copy()

    df["Accident_Date"] = pd.to_datetime(df["Accident_Date"], errors="coerce")

    month_lookup = {name: i + 1 for i, name in enumerate(MONTH_ORDER)}
    if "Month" not in df.columns:
        if "Month_Name" in df.columns:
            df["Month"] = df["Month_Name"].map(month_lookup)
        else:
            df["Month"] = df["Accident_Date"].dt.month
    ...
    if "Injury_Event" in df.columns:
        df["Injury_Event"] = df["Injury_Event"].astype(int)

    for col in ["No__of_Minimal", "No__of_Minor", "No__of_Major", "No__of_Fatal"]:
...
def load_and_clean(csv_path_or_buffer) -> pd.DataFrame:
    ...
    if fmt == "raw":
        ...
        return adapt_engineered(df)
    else:
        raise ValueError(build_schema_error(df))
